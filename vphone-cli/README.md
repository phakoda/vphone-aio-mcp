...
export PATH=/var/jb/usr/bin:/var/jb/usr/sbin:/var/jb/bin:/var/jb/sbin:/iosbinpack64/bin:iosbinpack64/usr/bin:/iosbinpack64/usr/local/bin:/usr/bin:/usr/sbin:/bin:/sbin
...
### Restore Patched Firmware

With the patched firmware ready, restore it to the virtual device.

```bash
idevicerestore -e iPhone17,3_26.1_23B85_Restore/
```

Once the restore completes, boot the VM back into DFU mode using `./boot_dfu.sh`.
